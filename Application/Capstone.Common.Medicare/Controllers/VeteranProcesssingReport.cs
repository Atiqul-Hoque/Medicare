...
using Capstone.Common.Medicare.ThirdParty.MedicareOnline.Veteran.Processing.Report;
...
namespace Capstone.Common.Medicare.Controllers
{
    ...
    [ApiController]
    public class DVAProcessingReport : ControllerBase
    {
        Guid TenantId;
        string locationId;
        [HttpPost]
        [Route("GetVeteranProcessingReport")]
        public async Task<string> CallVeteranProcessignReport(string TenantId, string locationId, string correlationId)
        {
            try
            {
                if (!String.IsNullOrEmpty(TenantId) || !String.IsNullOrEmpty(locationId))
                {
                    ...
                    return submitVeteranProcessingReport(DVAReportRequestType, correlationId);
        ...
        private string submitVeteranProcessingReport(DVAReportRequestType DVAReportRequestType, string dhs_correlationId)
        {
            ...
            try
            {
                ...
                VeteranProcessingReport processingReport = new VeteranProcessingReport(apiClient);
                Task<DVAProcessingReportResponseType> taskResp = processingReport.McpDvaProcessingReport(DVAReportRequestType, authorization, dhs_auditId, dhs_subjectId, "urn:uuid:" + dhs_messageId, dhs_auditIdType, "urn:uuid:"+dhs_correlationId, dhs_productId, dhs_subjectIdType, apiKey);
                taskResp.Wait();
                DVAProcessingReportResponseType DVAProcessingReportResponseType = taskResp.Result;
                string response = "";
                string status = "";
                if (DVAProcessingReportResponseType != null)
                {
                    response = JsonConvert.SerializeObject(DVAProcessingReportResponseType);
                    status = DVAProcessingReportResponseType.Status;
