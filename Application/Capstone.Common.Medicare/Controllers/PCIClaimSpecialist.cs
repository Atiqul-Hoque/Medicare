...
using Capstone.Common.Medicare.ThirdParty.MedicareOnline.PCI.Specialist;
...
namespace Capstone.Common.Medicare.Controllers
{
    ...
    public class PCIClaimSpecialist : Controller
    {
        private Guid TenantId;
        private string locationId;
        [HttpPost]
        [Route("SubmitClaim")]
        public async Task<ApiResponse<PatientClaimInteractiveResponseType>> CallPCISpecialistAsync(string tenantId, string locationId,string correlationId)
        {
            ...
            try
            {
                if (!String.IsNullOrEmpty(tenantId) && !String.IsNullOrEmpty(locationId))
                {
                    ...
                    return SubmitPCISpecialist(request, correlationId);
                }
                else
                {
                    apiResponse.Result = null;
        ...
        private ApiResponse<PatientClaimInteractiveResponseType> SubmitPCISpecialist(PatientClaimInteractiveRequestType patientClaimInteractiveRequestType,string correlationId)
        {
            var apiResponse = new ApiResponse<PatientClaimInteractiveResponseType>();
            ...
            requestLog.ServiceName = "PCI - specialist";
            Guid dhs_messageId = MedicareRequestLogHelper.LogRequest(requestLog);
            try
            {
                HttpClient apiClient = new HttpClient();
                    PCISpecialist patientClaimSpecialist = new PCISpecialist(apiClient);
                Task<PatientClaimInteractiveResponseType> taskResponse = patientClaimSpecialist.McpPatientClaimInteractiveSpecialist
