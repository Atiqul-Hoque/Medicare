...
using Capstone.Common.Medicare.ThirdParty.MedicareOnline.OEC;
...
namespace Capstone.Common.Medicare.Controllers
{
    ...
    [ApiController]
    public class OnlineEligibilityCheckClaim : ControllerBase
    {
        Guid TenantId;
        string locationId;
        [HttpPost]
        public async Task<ApiResponse<OnlineEligibilityCheckResponseType>> CallOnlineEligibilityCheck(string tenantId,string locationId)
        {
        ...
        private ApiResponse<OnlineEligibilityCheckResponseType> SubmitOEC(OnlineEligibilityCheckRequestType onlineEligibilityCheckRequestType)
        {
            ...
            try
            {
                ...
                OnlineEligibilityCheck onlineEligibilityCheck = new OnlineEligibilityCheck(apiClient);

                Task<OnlineEligibilityCheckResponseType> taskResp = onlineEligibilityCheck.McpEligibilityCheck(onlineEligibilityCheckRequestType, authorization, dhs_auditId, dhs_subjectId, "urn:uuid:" + dhs_messageId,dhs_auditIdType, "urn:uuid:" + dhs_correlationId, dhs_productId, dhs_subjectIdType, apiKey);
                taskResp.Wait();
